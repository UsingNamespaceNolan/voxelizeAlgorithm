...
void wndwsFormsGUI::Form1::button2_Click(System::Object^ sender, System::EventArgs^ e)
{
	...
	modelRange.highestX=-2147483647;
	modelRange.highestY=-2147483647;
	modelRange.highestZ=-2147483647;
	modelRange.lowestX=2147483647;
	modelRange.lowestY=2147483647;
	modelRange.lowestZ=2147483647;
...
void voxelizeVertices (long int** faces, float** vertices, RangeData modelRange, int numVertices, int numFaces, Vectors objectsAndVertices)
{
	...
	modelRange.yRange+=2;												//something like 5.8 becomes 5. the extra 0.8 may cause problems later on, so lets just add
	modelRange.zRange+=2;												//2(safety layer) and not worry about it.		
...
void voxelizeFaces (float** vertices, long int** faces, int faceVoxCount, int*** voxelArray, int objectNumber, Vectors objectsAndVertices)
{
    ...
    //when faces[x][COLUMNS]==-1, we know the face is a triangle because there are only 3 vertices
